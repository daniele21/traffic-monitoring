    ...
    private var overview: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            HStack(spacing: 12) {
                BrandMetricCard(
                    title: "Downloaded",
                    value: bytes(analytics.overallSummary.downloadedBytes),
                ...
                BrandMetricCard(
                    title: "Uploaded",
                    value: bytes(analytics.overallSummary.uploadedBytes),
                ...
                BrandMetricCard(
                    ...
                    value: analytics.peakPoint.map { bytes($0.totalBytes) } ?? "—",
                    detail: analytics.peakPoint.map { dateLabel($0.intervalStart) } ?? "No usage yet",
                    icon: "bolt.fill",
                    tint: BrandTheme.signalCyan
                )
                BrandMetricCard(
                    ...
                    icon: qualityIcon,
                    tint: BrandTheme.evidenceColor(for: analytics.evidenceQuality)
                )
            }
            ...
            BrandSectionHeader(
                ...
                subtitle: "A ranked view of where the selected period's measured traffic happened.",
                icon: "wifi.router"
            )
            ...
            if analytics.networkRows.isEmpty {
                ...
                BrandCard {
                    VStack(spacing: 0) {
                        ForEach(Array(analytics.networkRows.prefix(5)).enumerated(), id: \.element.id) { index, row in
                            networkRankRow(row, rank: index + 1)
                            if index < min(analytics.networkRows.count, 5) - 1 {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    HStack(spacing: 8) {
                        Text("Data quality")
                            .font(.headline)
                        BrandStatusPill(
                            text: analytics.evidenceQuality.title,
                            tint: BrandTheme.evidenceColor(for: analytics.evidenceQuality)
                        )
                    }
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                if let row = analytics.networkRows.first {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if row.isExpensive {
                            BrandStatusPill(text: "Hotspot", icon: "iphone.radiowaves.left.and.right", tint: BrandTheme.signalCyan)
                        }
                    }
                } else {
                    Text("No network usage yet")
                        .foregroundStyle(.secondary)
                }
            }
        }
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text(
                    analytics.store.persistsAcrossRelaunches
                        ...
                        : "The app will keep tracking, but historical analytics may not survive a relaunch until persistence recovers."
                )
                .font(.caption)
                .foregroundStyle(.secondary)
            }
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .bottom) {
                BrandSectionHeader(
                    ...
                    subtitle: "Follow the selected period and isolate a single network when you need to understand a spike.",
            ...
            HStack(spacing: 12) {
                BrandMetricCard(
                    title: "Total used",
                    value: bytes(analytics.trendSummary.totalBytes),
                    detail: selectedNetworkLabel,
                    icon: "sum",
                    tint: BrandTheme.networkBlue
                )
                BrandMetricCard(
                ...
                BrandMetricCard(
                    title: "Downloaded",
                    value: bytes(analytics.trendSummary.downloadedBytes),
                    detail: selectedNetworkLabel,
                    icon: "arrow.down",
                    tint: BrandTheme.networkBlue
                )
                BrandMetricCard(
                    title: "Uploaded",
                    value: bytes(analytics.trendSummary.uploadedBytes),
                    detail: selectedNetworkLabel,
                    icon: "arrow.up",
                    tint: BrandTheme.signalCyan
                )
    ...
    private var networks: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack(spacing: 12) {
                BrandMetricCard(
                    title: "Networks",
                    value: "\(analytics.networkRows.count)",
                    detail: "Seen in the selected period",
                    icon: "wifi.router",
                    tint: BrandTheme.networkBlue
                )
                BrandMetricCard(
                    title: "Top network",
                    value: analytics.networkRows.first.map { bytes($0.totalBytes) } ?? "—",
                    detail: analytics.networkRows.first?.networkName ?? "No usage yet",
                    icon: "chart.bar.fill",
                    tint: BrandTheme.signalCyan
                )
                BrandMetricCard(
                    title: "Hotspot-like",
                    value: "\(analytics.networkRows.filter(\.isExpensive).count)",
                    detail: "Networks marked expensive by macOS",
                    icon: "iphone.radiowaves.left.and.right",
                    tint: BrandTheme.signalCyan
                )
                BrandMetricCard(
                    title: "Needs identification",
                    value: "\(analytics.networkRows.filter { analytics.identityQuality(for: $0) != .identified }.count)",
                    detail: "Networks with partial or unknown identity",
            ...
            BrandSectionHeader(
                title: "Usage by network",
                subtitle: "Compare usage, connection type, evidence quality, and recency. Open any network to rename it or inspect its history.",
                icon: "network"
            )
            ...
            HStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 7) {
                        ...
                        if row.isExpensive {
                            BrandStatusPill(text: "Hotspot", icon: "iphone", tint: BrandTheme.signalCyan)
                        }
                        if row.isConstrained {
                            BrandStatusPill(text: "Constrained", icon: "gauge.with.dots.needle.33percent", tint: BrandTheme.warning)
                        }
                    }

                    ProgressView(value: networkShareValue(row))
                        .tint(BrandTheme.networkBlue)
                        .frame(maxWidth: 360)
            ...
            VStack(alignment: .leading, spacing: 3) {
                HStack(spacing: 6) {
                    Text(row.networkName)
                        .fontWeight(.medium)
                    if row.isExpensive {
                        BrandStatusPill(text: "Hotspot", tint: BrandTheme.signalCyan)
                    }
                }
